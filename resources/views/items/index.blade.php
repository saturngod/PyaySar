    ...
    <main class="flex-1 overflow-y-auto">
        ...
        <div class="p-6">
            ...
            @if($items->count() > 0)
                ...
                <x-table
                    :headers="['Name', 'Description', 'Price', 'Currency', 'Actions']"
                    caption="Items List ({{ $items->count() }} items)"
                    :paginator="$items"
                >
                    @foreach($items as $item)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-900">{{ $item->formatted_price }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <x-badge size="sm">{{ $item->currency }}</x-badge>
                            </td>
                ...
                {{-- Manual pagination for debugging --}}
                @if($items->hasPages())
                    <div class="mt-4 px-6 py-4 bg-white border-t border-gray-200 rounded-lg shadow-sm">
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-700">
                                Showing latest items ({{ $items->count() }} on this page)
                            </div>
                            <div class="flex items-center space-x-2">
                                @if($items->previousPageUrl())
                                    <a href="{{ $items->previousPageUrl() }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                ...
                                @if($items->hasMorePages())
                                    <a href="{{ $items->nextPageUrl() }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                        Next
                ...
                <x-card>
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-500 mb-6">
                            {{ request('search') ? 'No items match your search criteria.' : 'Get started by creating your first item.' }}
                        </p>
                        <div class="space-x-4">
                            @if(request('search'))
                                <a href="{{ route('items.index') }}" class="px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors duration-200">
